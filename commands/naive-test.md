...
Needs the Playwright MCP server (browser tools) — see the project README if `browser_*` tools
aren't available. Run it on demand while developing, or dispatch it as a subagent to run in
parallel. It complements scripted e2e tests: those assert known flows; this discovers unknown gaps.
